...
combined['PC1(listening intensity)'], combined['PC2(interaction style)'] = X_pca[:, 0], X_pca[:, 1]
...
plt.figure()
sns.scatterplot(data=combined, x='PC1(listening intensity)', y='PC2(interaction style)', hue='person', alpha=0.5)
plt.title('Actual Labels (Identity)')
plt.show()

# Plot K-Means results
plt.figure()
sns.scatterplot(data=combined, x='PC1(listening intensity)', y='PC2(interaction style)', hue='cluster', palette='viridis', alpha=0.5)
